export function marbleVertShader(){
    return`
    varying vec2 vUv;
    void main()
    {
...
export function marbleFragShader(){
    return`
    const float PI = 3.14;
...
export function cloudFragShader() {
    return `
uniform vec3      iResolution;
uniform vec4      iMouse; 
uniform float     iTime;
...
information isn't used to rendering or shading but only as an underlying volumedistance to 
determine dynamic step size, by carefully selecting an equation(polynomial for speed) to 
step as a function of overall density (not necessarialy rendered) the visual results can be 
thesame as a naive implementation with ~40% increase in rendering performance.
